class Admin::PatientsController < Admin::BaseController
  ...
  def index
    ...
    if @term
      @patients = @patients.where('UPPER(patients.firstname) LIKE ? OR UPPER(patients.lastname) LIKE ? OR patients.phone_primary LIKE ? OR UPPER(patients.email_primary) LIKE ?', "#{@term.upcase}%", "#{@term.upcase}%", "#{@term}%", "#{@term.upcase}%")
    end
  ...
  def create
    ...
    respond_to do |format|
      if @patient.save
        ...
        format.json { render :show, status: :created, location: admin_patient_path(@patient) }
      else
