...
class Pharmacy::SubscriptionsController < Pharmacy::BaseController
  ...
  def index
    @active_subscription = current_pharmacy.active_subscription
    @payments = current_pharmacy.payments
    @subscriptions = current_pharmacy.subscriptions.order(created_at: :desc)
    
    @active_card = Stripe::Customer.retrieve(current_pharmacy.customer_id).sources.data.first rescue nil
    ...
    respond_to do |format|
      ...
      format.csv do
        ...
        send_data content, 
          ...
          :disposition => "attachment; filename=digiscript_pharmacy_subscriptions_#{ Time.now.strftime('%Y-%m-%d') }.csv" 
      end
    end
  end
  ...
  def new
    ...
    @plans = Plan.published.available_to_pharmacy
  end

  def create
    if @active_subscription.blank? || @active_subscription.renewable?
      
      @subscription = current_pharmacy.subscriptions.build(subscription_params)
  ...
  def destroy
    ...
    redirect_to pharmacy_subscriptions_path
  end
  
  private
  
  def load_plan
    @plan = Plan.published.available_to_pharmacy.find(params[:plan_id] || session[:subscribe_plan_id]) rescue Plan.published.available_to_pharmacy.default.random.first
  end
  ...
  def load_active_subscription
    if current_pharmacy.active_subscription
      @active_subscription = current_pharmacy.active_subscription
    elsif current_pharmacy.subscriptions.last
      @active_subscription = current_pharmacy.subscriptions.last
    end
  end
